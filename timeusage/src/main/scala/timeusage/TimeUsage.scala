...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val pri: List[Column] = List()
    val work: List[Column] = List()
    val other: List[Column] = List()
    val priPattern = "t01*|t03*|t11*|t1801*|t1803*".r
    val workPattern = "t05*|t1805*".r
    for (s <- columnNames) {
      s match {
        case workPattern => s :: work
        case priPattern => s :: pri
        case _ => s :: other
      }
    }
    (pri, work, other)
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = ageprj(col("teage")).as("age")
